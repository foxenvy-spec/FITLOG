          ...
          {muscleRecommendation ? (
            <>
              ...
              {isRestDay ? (
                ...
                // description ให้กระชับกว่านี้" — ประโยคเดิม "วันนี้เหมาะกับการพักและฟื้นตัว — ฝึกมา N
                // วันในสัปดาห์นี้แล้ว" (~56 ตัวอักษร) กระชับลงเหลือ "วันนี้ควรพักฟื้นตัว — ฝึกมาแล้ว N
                // วันสัปดาห์นี้" (~43 ตัวอักษร, -23%) ไม่ตัดข้อมูลอะไรออก (ยังบอกคำแนะนำหลัก + จำนวนวันที่
                // ฝึกมาแล้วเหมือนเดิมทุกอย่าง) แค่สั้นกระชับกว่าเดิม — คงลำดับ "คำแนะนำหลักก่อน เหตุผลตามหลัง"
                // ไว้เหมือนเดิมตามที่แก้ไปแล้วด้านบน (กัน ellipsis ตัดโดนส่วนสำคัญ)
                <p className="truncate mt-0.5 font-medium" style={{ fontSize: 11, color: TEXT.title }}>
                  {thisWeekWorkoutDays != null && thisWeekWorkoutDays > 0
                    ? `วันนี้ควรพักฟื้นตัว — ฝึกมาแล้ว ${thisWeekWorkoutDays} วันสัปดาห์นี้`
                    : 'วันนี้ควรพักฟื้นตัว'}
                </p>
              ) : makeupSessionActive ? (
