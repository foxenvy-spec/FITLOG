...
export default function ProgramPage() {
  ...
  return (
    <div className="space-y-5 lg:max-w-3xl lg:mx-auto">
      ...
      {!isToday && (
        // P2-03 — ข้อความในเครื่องหมายคำพูดเคย quote ป้ายปุ่มเก่า "บันทึกเข้า Log" ทั้งที่ปุ่มจริงด้านล่าง
        // (L989-1007) ถูกเปลี่ยนป้ายไปเป็น "บันทึกแผนนี้เข้า Log วันนี้ (N ท่า)" แล้วในรอบ design-review
        // อีกรอบหนึ่ง — สอง string นี้ hardcode แยกกันคนละที่ ไม่มี source of truth เดียว จึง drift กัน
        // อัปเดตให้ quote ตรงกับป้ายปุ่มจริง (ไม่รวมเลขจำนวนท่าซึ่งเป็นค่าที่เปลี่ยนได้ ไม่เหมาะใส่ในประโยคนี้)
        <p className="text-[12px] text-muted bg-surface2 rounded-lg px-3 py-2">
          กำลังดูแผนของวัน{WEEKDAYS[selectedDow]} — ถ้ากด &quot;บันทึกแผนนี้เข้า Log วันนี้&quot; จะถูกบันทึกลงวันที่จริงคือวันนี้เสมอ
        </p>
      )}
