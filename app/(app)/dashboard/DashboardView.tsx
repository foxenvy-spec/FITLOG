...
export default function DashboardPage() {
  ...
  return (
    ...
    <div
      ...
      className="flex overflow-x-auto overflow-y-hidden snap-x snap-mandatory no-scrollbar items-start"
      style={{ scrollBehavior: 'smooth', height: detailTrackHeight, transition: 'height 200ms ease' }}
    >
    <div ref={slide1Ref} className="shrink-0 w-full snap-center">
                ...
                {weightPct !== null && (
                  <div>
                    ...
                    {/* ฟีดแบ็ก (design review, micro-polish หลังปิด P1) "'7% Progress · เหลืออีก 6.5 kg'
                        ...
                        ชัดกว่านี้ ไม่ต้องเพิ่มข้อมูลใหม่" — แยก headline (ตัวเลข % หรือ "เริ่มต้นเป้าหมาย")
          ...
          <div className="mt-4">
            ...
            <div className="min-w-0 max-w-[230px] relative">
              ...
              {todayCompleted ? (
                ...
                // 'ดูคำแนะนำเพิ่มเติม →' แล้ว (ไม่มีปุ่มเริ่มเหมือนกัน) ไม่มี action ที่ชวนฝึกซ้ำในสล็อตนี้
                // — แต่ฟีดแบ็กรอบถัดมา "ผู้ใช้จะสงสัยธรรมชาติว่า 'ฝึกอะไรไปบ้าง' โดยเฉพาะตอนแผนวันนี้
                // (Pull) กับสิ่งที่ฝึกจริง (อก+ไหล่) ไม่ตรงกัน — ไม่มีทางดูรายละเอียดจะต้องไปงมเองใน
                // /stats หรือ /calendar" เพิ่มลิงก์ข้อความล้วน (ไม่ใช่ปุ่ม กันตีความว่าเป็น CTA ชวนฝึก)
    ...
    {/* ฟีดแบ็ก (design review — "Recovery มีข้อมูลซ้ำกับ MINT Coach 3 รอบ") "Recovery card ควรเป็นเจ้าของแค่
          ...
          <div className="flex items-center gap-4">
            ...
            >
              {/* ฟีดแบ็ก "86% Goal ดูไม่สัมพันธ์กับข้อความข้างๆ (3 ครั้ง/อีก 2 ครั้งถึงเป้าหมาย) — 3/5 ครั้ง
                  ...
                  ที่ฝึกจริงๆ แต่เป็นค่าเฉลี่ย % ของเป้าหมายเซ็ต/สัปดาห์ข้ามทุกกล้ามเนื้อ คนละ metric กับ
                  "3 ครั้งแล้ว/อีก 2 ครั้ง" (นับจำนวนวันที่ฝึก) ที่วางอยู่ข้างๆ กันพอดี — ป้าย "Goal" เดิมกำกวม
                  ทำให้ดูเหมือนเป็น metric เดียวกัน แก้ด้วยการเปลี่ยน label ให้ตรงกับสิ่งที่วัดจริง (ไม่แตะสูตร
                  คำนวณ weeklyGoalPct เอง เพราะใช้ร่วมกับ Fitness Score weighting อยู่ด้วย เปลี่ยนความหมาย
                  ตรงนั้นจะกระทบวงกว้างกว่านี้มาก)
                  ฟีดแบ็ก (design review รอบถัดมา, P1) "77% Volume กับ Weekly Sets 82 sets ใต้ๆ กันใช้คำว่า
                  Volume คนละความหมาย — ต้องเช็ค semantics ให้ตรงกัน" — ตรวจแล้วพบว่า weeklyGoalPct เป็น %
                  ของเป้าหมาย "เซ็ต" ต่อกลุ่มกล้ามเนื้อ ไม่ใช่ training volume จริง (kg x reps x sets) ตอนที่
                  เปลี่ยนจาก "Goal" เป็น "Volume" ครั้งก่อน (comment ด้านบน) เข้าใจผิดว่า "Volume-based" =
                  ใช้คำว่า Volume ได้ ทั้งที่จริงคือ Set-based — เปลี่ยนเป็น "Set Goal" แทน ยังคงแยกจาก "3
                  ครั้งแล้ว" (workout count) ข้างๆ ได้เหมือนเดิม แค่ไม่ใช้คำว่า Volume ผิดความหมายอีกต่อไป
                  (ตรงกับกฎ "Volume ต้องเป็น kg x reps x sets เท่านั้น" ที่ใช้แก้จุดอื่นทั่วแอปมาแล้วในรอบก่อน) */}
              <GoalRing
      ...
      {/* end slide 1 (whole dashboard grid above, untouched internally) */}
