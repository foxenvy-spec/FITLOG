...
export const dashboardSpec = {
  ...
  workoutCard: {
    ...
    // v48: ฟีดแบ็ก "Ring 100% ใหญ่มาก ลดลง 10% แล้วเพิ่มรูป Dumbbell ให้เด่นขึ้น จะบาลานซ์กว่า" —
    // 76 -> 68 (-10% ตามที่ขอเป๊ะ) ไอคอนดัมเบลข้างใน (TodaysWorkoutCompactCard.tsx) ปรับขนาดคู่กันจาก
    // 56 -> 52px แทนที่จะคงเดิม (56px ที่ inner diameter เหลือแค่ 58px จะบีบจนเกือบชนขอบวง) แต่สัดส่วน
    // ไอคอนต่อวง (52/68 = 76%) ยังสูงกว่าเดิม (56/76 = 74%) จริง — ผลรวม: วงเล็กลง + ดัมเบลเด่นขึ้นเทียบ
    // กับวง ตามที่ขอทั้งสองอย่างพร้อมกัน
    ringSize: 68,
  },
  healthBanner: {
    height: 66, // -11% จาก 74
