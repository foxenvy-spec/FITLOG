...
// เดิม (suggestNextPR) ใช้น้ำหนักที่หนักที่สุด "ตลอดกาล" ของท่านั้นเป็นฐาน แล้วบวกเพิ่มตายตัวเสมอ
// (เช่น +2.5kg) ไม่ว่าฟอร์มล่าสุดจริงจะเป็นอย่างไร — ทำให้แนะนำหนักขึ้นเรื่อยๆ แม้เซสชันหลังๆ reps จะ
// ตกลงมากแล้วก็ตาม (ไม่รู้จัก "ถอย") เปลี่ยนมาใช้หลัก double progression: ดูเซสชัน "ล่าสุด" (ไม่ใช่ PR
// ตลอดกาล) เทียบกับช่วง reps เป้าหมาย — ถ้าทำได้ถึง/เกินบนสุดของช่วงแล้ว ค่อยแนะนำเพิ่มน้ำหนัก (พร้อมลด
// ช่วง reps เป้าหมายลงมาที่ล่างของช่วงเดิม ตามหลักที่ว่าน้ำหนักมากขึ้น reps ที่ทำไหวย่อมลดลง) ถ้ายังไม่ถึง
// บนสุด แนะนำคงน้ำหนักเดิมแล้วไล่เพิ่ม reps ก่อน — ไม่มีข้อมูล target reps ต่อท่าจากโปรแกรมมาให้ใช้ค่า
// เริ่มต้น 8-12 reps (ช่วงมาตรฐานสำหรับ hypertrophy ที่ใช้กันทั่วไป)
export interface ProgressiveOverloadSuggestion {
  exerciseName: string
  lastWeight: number
  lastReps: number
  lastSets: number
  // increaseWeight: ทำ reps ถึง/เกินบนสุดของช่วงเป้าหมายแล้ว พร้อมขยับน้ำหนักขึ้น
  // addReps: ยังไม่ถึงบนสุด แนะนำคงน้ำหนักเดิม ไล่เพิ่ม reps ก่อน
  action: 'increaseWeight' | 'addReps'
  targetWeight: number
  targetRepsLow: number
  targetRepsHigh: number
}

const DEFAULT_TARGET_REPS: { low: number; high: number } = { low: 8, high: 12 }

export function suggestProgressiveOverload(
  exerciseName: string,
  allTimeEntries: Workout[],
  exercises: ExerciseDef[] = [],
  targetReps: { low: number; high: number } = DEFAULT_TARGET_REPS
): ProgressiveOverloadSuggestion | null {
  const entries = allTimeEntries.filter(
    (w) => w.type === 'strength' && w.exercise_name === exerciseName && w.weight_kg != null && w.reps != null
  )
  if (entries.length === 0) return null

  const last = [...entries].sort((a, b) => b.performed_at.localeCompare(a.performed_at))[0]
  const lastWeight = last.weight_kg ?? 0
  const lastReps = last.reps ?? 0
  const lastSets = last.sets ?? 1

  const known = exercises.find((ex) => ex.name === exerciseName || ex.nameTh === exerciseName)
  const increment = known?.equipment === 'ดัมเบล' ? 1 : 2.5

  if (lastReps >= targetReps.high) {
    return {
      exerciseName,
      lastWeight,
      lastReps,
      ...
      targetRepsLow: targetReps.low,
      targetRepsHigh: Math.max(targetReps.low, targetReps.high - 2),
    }
  }
  ...
  return {
    ...
    targetRepsLow: Math.min(targetReps.high, lastReps + 1),
    targetRepsHigh: Math.min(targetReps.high, lastReps + 2),
  }
}
