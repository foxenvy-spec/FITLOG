...
// รูปพื้นหลัง Today's Workout แต่วาดด้วย CSS ล้วน (ไม่ต้องมี asset เพิ่ม) ใช้ opacity ของสีขาวต่ำมาก (2.5%)
export const DIAGONAL_TITANIUM_CSS =
  'repeating-linear-gradient(115deg, rgba(255,255,255,.025) 0px, rgba(255,255,255,.025) 1px, transparent 1px, transparent 40px)'

// แสงส้ม ambient จาง ๆ แผ่ขึ้นจากโซนที่ Workout Card อยู่ (ธีมไฟของแอป) ไปสัมผัส Metric Card/Focus Card
// เบาๆ — วางไว้เป็น radial-gradient ใหญ่ตำแหน่งค่อนไปทางล่างของหน้า (~65%) กระจายกว้าง/เบลอมาก
// (opacity สูงสุดแค่ ~3.5% ตรงจุดกำเนิด แล้วจางเป็น transparent ก่อนถึง 55% ของรัศมี) ให้ความรู้สึกแสงลอด
// มาแตะขอบล่างของการ์ดด้านบนเท่านั้น ไม่ใช่ spotlight ชัดเจน — ไม่ผูกกับตำแหน่งพิกเซลจริงของการ์ด (เพราะ
// ความสูงเนื้อหาผันแปรตามข้อมูล) แค่ตั้งใจให้ "รู้สึกได้แต่ไม่รู้ตัว" ตามที่ขอ
export const AMBIENT_ORANGE_CSS =
  'radial-gradient(80% 55% at 65% 68%, rgba(255,138,0,.035), transparent 55%)'
